...
TrafficLight::TrafficLight(int redPin1, int yellowPin1, int greenPin1, int redPin2, int yellowPin2, int greenPin2, long stateTime[])
    : _redPin1(redPin1), _yellowPin1(yellowPin1), _greenPin1(greenPin1), _redPin2(redPin2), _yellowPin2(yellowPin2), _greenPin2(greenPin2), _nextChange(0), _state(0)
{
    _pStateTime = stateTime;
}

void TrafficLight::setup(long startTime)
{
...
void TrafficLight::update(long now)
{
    if (now < _nextChange)
        return;
    {
